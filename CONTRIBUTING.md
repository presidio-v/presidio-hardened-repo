# Contributing to presidio-hardened-repo
...
Issues labelled [`good first issue`](https://github.com/presidio-v/presidio-hardened-repo/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22)
...
`pyproject.toml` under `[tool.ruff]` — line length 100, target `py311`, and the hardened
family rule set (`E, F, W, I, N, UP, S, B, A, C4, SIM, TCH`). `S101` is ignored (asserts are
the test idiom) and `S603`/`S607` are ignored because the tool shells out to `git`/`gh` by
design; do not add new `# noqa` suppressions to widen that.
...
- **`scripts/gh_settings.py`** — mutates GitHub repository and organisation settings (branch
  protection, visibility, collaborators, labels). A change here must not weaken the
  dry-run-by-default posture or the `--yes` gate on irreversible actions.
- **`scripts/render.py`** — writes rendered files into target repositories and defines the
  token/FILL contract. A change must not weaken the fail-hard behaviour on unresolved or
  undefined tokens, or let FILL markers pass as resolved.
- **`scripts/spdx_headers.py`** — writes headers into target source files. Keep the `--check`
  guard and the idempotent insert behaviour intact.
- **`templates/sheets/` answer-sheet skeletons** — these render into attestations. A change
  that could induce an over-claim (asserting a criterion the evidence does not support)
  needs the security-reasoning bar below.
...
The scripts and tests are the source tree for this project (there is no `src/` package); lint
and format cover `scripts/` and `tests/`.
